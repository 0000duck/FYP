...
#include </home/nvidia/catkin_ws/src/trajectory_generator/cfg/cpp/trajectory_generator/TrajectoryGeneratorUIConfig.h>
...
double traj_vel;
double traj_acc;
double fast_mode_vel;
double fast_mode_acc;
...
void trigger_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
  if (!is_odom)
    return;
  // if (control_mode == VELOCITY_MODE)
  if (control_mode == POSITION_MODE || control_mode == HOVER_MODE)
  {
    hover_des[0] = p(0);
    hover_des[1] = p(1);   
    hover_des[2] = p(2);
    hover_des[3] = yaw;
    control_mode = HOVER_MODE;
    ROS_INFO_THROTTLE(10, "Reset hovering point @[%3.2f, %3.2f, %3.2f; %3.2f%%]\n",
      hover_des[0], hover_des[1], hover_des[2], hover_des[3]*180.0/3.14159);
  }
}
...
void waypoints_callback(const nav_msgs::Path::ConstPtr &msg)
{
  ...
  // Decode waypoints
  ROS_WARN("waypoints callbck 1");
  waypoints.clear();
  waypoints.resize(msg->poses.size(), zeros<colvec>(4));
  ROS_WARN("waypoints callbck 2");
  ...
  ROS_WARN("waypoints callbck 2.5");
  ...
  ROS_WARN("waypoints callbck 2.6");
...
ROS_WARN("waypoints callbck 3");
  ...
  if (msg->header.frame_id.compare(std::string("fast"))==0)
  {
    traj_gen.SetVelocity(fast_mode_vel);
    traj_gen.SetAcceleration(fast_mode_acc);
  }

  ROS_WARN("waypoints callbck 4");
  traj_gen.SetWaypoints(p, v, a, yaw, dyaw, t, _waypoints); 
  
  if (msg->header.frame_id.compare(std::string("fast"))==0)
  {
    traj_gen.SetVelocity(traj_vel);
    traj_gen.SetAcceleration(traj_acc);
  }
 ROS_WARN("waypoints callbck 5");
  ...
  ROS_WARN("waypoints callbck end");
}
...
void visualization()
{
  ...
  nav_msgs::Path path;
  path.header.stamp    = t;
  path.header.frame_id = string("world");
  ...
  nav_msgs::Path traj;
  traj.header.stamp    = t;
  traj.header.frame_id = string("world");
...
int main(int argc, char** argv)
{
  ...
  n.param("traj_vel", traj_vel , 1.5);   
  n.param("traj_acc", traj_acc , 1.0);   
  n.param("fast_mode_vel", fast_mode_vel , traj_vel);   
  n.param("fast_mode_acc", fast_mode_acc , traj_acc);   
  ...
  ros::Subscriber sub2 = n.subscribe("rc",         10, rc_callback);        
  ros::Subscriber sub3 = n.subscribe("trigger",    10, trigger_callback);          
  ros::Subscriber sub4 = n.subscribe("waypoints",  10, waypoints_callback); 
  ros::Subscriber sub5 = n.subscribe("correction", 10, correction_callback);               
