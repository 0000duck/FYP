...
bool           is_init_takeoff = false;
colvec         v0 = zeros<colvec>(3); 
ros::Time      t0;
// Max de-acceleration for online recovery (stabilization)
double         a0 = 2;
...
{
  // Only continue when both state estimation and RC are activated
  is_odom = true;  
  if (!is_rc)
  {
    is_init_takeoff = false;
    control_mode = TAKEOFF_MODE;
    return;
  }
  ...
  {  
    if (!is_init_takeoff)
    {
      is_init_takeoff = true;
      init_yaw = yaw;
      v0 = v;
      t0 = msg->header.stamp;
    }
    double dt  = (msg->header.stamp - t0).toSec();
    double mt = norm(v0, 2) / a0;
    colvec vc = zeros<colvec>(3);
    if (dt > mt)
      vc = zeros<colvec>(3);
    else
      vc = v0 * (1 - dt / mt);
    position_cmd.position.x     = p(0);
    position_cmd.position.y     = p(1);
    position_cmd.position.z     = p(2);    
    position_cmd.velocity.x     = vc(0);
    position_cmd.velocity.y     = vc(1);
    position_cmd.velocity.z     = vc(2);    
