...
void N1CtrlFSM::process_control(const ros::Time& now_time)
{
	...
	if (idling_state == NOIDLING)
	{
		if ((state==JS_CTRL && axis_states[2]==FIX) // JS_CTRL hovering case
...
void N1CtrlFSM::process_no_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	u.roll = 0.0;
	u.pitch = 0.0;
...
void N1CtrlFSM::process_raw_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	double des_yaw = yaw_add( get_yaw_from_odom(), -toRad(rc_data.yaw * param.rc.yaw_scale));
	
	u.roll  =  toRad(rc_data.roll * param.rc.attitude_scale);
	u.pitch = -toRad(rc_data.pitch * param.rc.attitude_scale);
	u.yaw   = des_yaw;
...
void N1CtrlFSM::process_idling_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3, double idling_lasting_time)
{
	u.roll = 0.0;
	u.pitch = 0.0;
	u.yaw = get_yaw_from_odom();
...
void N1CtrlFSM::process_hover_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	Desired_State_t des;
...
void N1CtrlFSM::process_break_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	Desired_State_t des;
	des.p = odom_data.p;
	des.v = Vector3d::Zero();
...
void N1CtrlFSM::process_cmd_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	Desired_State_t des;
	des.p = cmd_data.p;
	des.v = cmd_data.v;
...
void N1CtrlFSM::process_js_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	Desired_State_t des;
	Vector3d des_v;
	double des_dyaw;
