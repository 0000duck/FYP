...
HovThrKF::HovThrKF(Parameter_t& param_):
	param(param_)
{
...
void HovThrKF::init()
{
	...
	x = Vector2d(0.0, 0.0);
	P = Matrix<double, 2, 2>(); P << 0.5*0.5, 0, 0, 1*1;
	Q = Matrix<double, 2, 2>(); Q << 0.1*0.1, 0, 0, 1*1;
	F = Matrix<double, 2, 2>(); F << 1, 0, -max_force/mass, 0;
	B = Matrix<double, 2, 1>(); B << 0, max_force/mass;
	H = Matrix<double, 1, 2>(); H << 0, 1;
	R = Matrix<double, 1, 1>(); R << 0.001*0.001;
}
...
// printf("hov = %.3f a = %.3f P =[ %5e, %5e;  %5e, %5e]\n---------------------\n",
	...
	uav_utils::limit_range(x(0), 0.5, 0.9);
}
...
void HovThrKF::simple_update(Eigen::Quaterniond q, double u, Eigen::Vector3d acc)
{
	...
	x(0) = x(0) + compensate;
	uav_utils::limit_range(x(0), 0.3, 0.9);
	// printf("u=%f param.full_thrust=%f mass=%f", u, param.full_thrust, param.mass);
	// printf("da=%2.3f ba=%2.3f c=%2.6f x=%.2f\n", 
	// 	acc_des(2), acc_body(2), compensate, x(0));
}
